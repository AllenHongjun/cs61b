...
/**
 * The TETile object is used to represent a single tile in your world. A 2D array
 * of tiles make up a
 * board, and can be drawn to the screen using the TERenderer class.
 *
 * All TETile objects must have a character. textcolor, and background clor to be
 * used to represent
 * the tile when drawn to the screen. You can also optionally provide a path to an
 * image file of an appropriate size(16*16) to be drawn in place of the unicdoe representation. If the image path
 * provided cannot be found, draw will fallback to using the provided character and color representation, so
 * you are free to use image tiles tiles on your own computer.
 *
 * The provided TETile is immutable, i.e none fo its instance variables can changes. You are welcme
...
public class TETile {
    ...
    /**
     * Full constructor for TETile ojbects.  瓦片
     * @param character The character displayed on the screeen.
     ...
     * @param filepath Full path to image to be used for this tile. Must be correst size (16 * 16)
     */
     ...
     * not event try to draw an image, and will instead use the provide character and colors.
     * @param character
     * @param textColor
    ...
    /**
     * 类类型的参数 可以作为自身函数的 参数 和返回 带入到自己类内部的函数当中来处理
    ...
    /**
     * Draws the tile to the screen at location x, y. If a valid failepath is proviced,
     ...
     *
     * Note that the image provided must be of the right size(16* 16). It will not be
     ...
     * @param dg the maximum differnce in green value
     ...
     * print in what minght seem like backwards order(so that the 0th row gets
     * printed last).
